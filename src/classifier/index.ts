...
const STYLING_ATTRIBUTES = new Set(['className', 'class', 'style', 'sx', 'css', 'tw', 'cs']);
...
const isCosmeticAttribute = (name: string): boolean =>
  COSMETIC_ATTRIBUTES.has(name) || name.startsWith('data-') || name.startsWith('aria-');
    ...
    switch (node.getKind()) {
      ...
      case SyntaxKind.MethodDeclaration: {
        const hasInlineUseServer =
          node.getText().includes('"use server"') || node.getText().includes("'use server'");
...
const isStylingPropForwarding = (changedLines: Set<number>, ranges: LineRange[]): boolean => {
...
const classifyByRegions = (
  ...
  if (isStylingOnly) {
    ...
    if (impact === 'visibility')
      return {
        action: 'LIGHTWEIGHT',
        reason: 'Styling change affects element visibility — verify elements still render',
      };
    if (impact === 'layout')
      return {
        action: 'LIGHTWEIGHT',
  ...
  if (all.every((r) => ['props', 'types', 'imports', 'exports'].includes(r))) {
    if (regions.has('props'))
      return { action: 'LIGHTWEIGHT', reason: 'Prop interface changed — verify rendered output' };
  ...
  if (
    ...
    return {
      action: 'LIGHTWEIGHT',
      reason:
        'Styling prop forwarded (className/style pass-through) — verify component still renders',
    };
  }
  ...
  if (all.every((r) => ['jsx-markup', 'jsx-styling', 'jsx-cosmetic', 'imports'].includes(r))) {
    ...
    if (impact === 'visibility')
      return {
        action: 'LIGHTWEIGHT',
        reason: 'JSX change affects visibility — verify elements still render',
      };
    if (impact === 'layout')
      return {
        action: 'LIGHTWEIGHT',
        reason: 'JSX change affects layout — verify elements are reachable',
      };
    return {
      action: 'LIGHTWEIGHT',
      reason: 'JSX structure changed — verify elements render correctly',
    };
  }
  ...
  if (all.every((r) => ['event-handler', 'imports', 'jsx-cosmetic'].includes(r))) {
    return {
      action: 'FULL_QA',
      reason: 'Event handler logic changed — test user interactions and outcomes',
    };
  }
  ...
  if (all.every((r) => ['async-logic', 'imports'].includes(r))) {
    return {
      action: 'FULL_QA',
      reason: 'Async logic changed — test loading, error, and data states',
    };
  }

  // Conditional render only — what shows/hides changed, not how interactions work
  if (
    all.every((r) => ['conditional-render', 'jsx-markup', 'jsx-styling', 'imports'].includes(r))
  ) {
    return {
      action: 'LIGHTWEIGHT',
      reason: 'Conditional render changed — verify correct state is shown',
    };
  }
  ...
  if (
    ...
    return {
      action: 'FULL_QA',
      reason: 'Event handler and render logic changed — test interactions and state',
    };
  }
  ...
  if (regions.has('server-action')) {
    return {
      action: 'FULL_QA',
      reason: 'Server action changed — test form submission, redirects, and auth',
    };
  }
  ...
  if (regions.has('hook-deps') && !regions.has('function-body')) {
    return {
      action: 'FULL_QA',
      reason: 'Hook dependency changed — test side-effects and state transitions',
    };
  }
...
export const classifyFile = (file: StagedFile): ClassificationResult => {
  ...
  if (file.status === 'D')
    return { action: 'SKIP', reason: 'File deleted — no browser behaviour to test' };
  if (TRIVIAL_EXTENSIONS.has(ext))
    return { action: 'SKIP', reason: `Asset file (${ext}) — no behaviour to test` };
  if (file.status === 'R' && !file.diff.trim())
    return { action: 'SKIP', reason: 'Pure rename — no content changed' };
  if (DATA_EXTENSIONS.has(ext))
    return { action: 'SKIP', reason: `Config/data file (${ext}) — no browser behaviour` };
  if (!CODE_EXTENSIONS.has(ext))
    return { action: 'SKIP', reason: `Non-JS/TS file (${ext}) — skipping` };
...
export const classifyStagedFiles = (files: StagedFile[], skipTrivial = true): ClassifiedFile[] =>
  files
