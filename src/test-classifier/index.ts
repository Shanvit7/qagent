...
const HARD_FAIL_ISSUES = new Set<PreCheckIssue>([
  "no-tests",
  "no-goto",
]);
...
const hasTestBlocks = (code: string): boolean =>
  /\btest\s*\(/.test(code);

const hasPageGoto = (code: string): boolean =>
  /\bpage\.goto\s*\(/.test(code);
...
const hasOnlyVisibilityAssertions = (code: string): boolean => {
  ...
  const deepAssertions = assertCalls.filter((a) =>
    !/\.toBeVisible\s*\(|\.toBeEnabled\s*\(|\.toBeDisabled\s*\(|\.toBeChecked\s*\(/.test(a),
  );
  return deepAssertions.length === 0;
};

const hasCssSelectors = (code: string): boolean =>
  /page\.locator\s*\(\s*['"`][.#]/.test(code) ||
  /\[data-testid/.test(code);

const hasFlakyTimeout = (code: string): boolean =>
  /waitForTimeout\s*\(/.test(code);
