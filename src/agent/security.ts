  ...
  if (found.length === 0) {
    try {
      return readdirSync(cwd, { withFileTypes: true })
        .filter((e) => e.isDirectory() && !["node_modules", ".git", ".next", "dist", "build", ".qagent"].includes(e.name))
        .slice(0, 6)
        .map((e) => e.name)
        .join(" ") || ".";
    } catch {
...
const runSecurityAgent = async (
  ...
  while (toolCallCount < MAX_CALLS && Date.now() < deadline) {
    const response = await chat(aiConfig, messages, { tools: TOOLS as ToolDef[], temperature: 0.1 });
